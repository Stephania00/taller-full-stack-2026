// productos.js — Sprint 3
// Maneja dos páginas:
//   productos.html      → catálogo público (fetch GET /productos)
//   admin-productos.html → panel CRUD (GET / POST / PUT / DELETE)
    ...
    .catch(() => {
      ...
      if (tbody) tbody.innerHTML = `
        <tr><td colspan="6" class="text-center text-danger py-4">
          ...
          No se pudo conectar con el backend. Verificá que server.js esté corriendo.
        </td></tr>`;
    });
}
...
function guardarProducto(e) {
  ...
  fetch(url, {
    ...
    .catch(() => mostrarMensaje('Error al conectar con el servidor.', 'text-danger'));
}
...
function eliminarProducto(id, nombre) {
  ...
  fetch(`${API_URL}/productos/${id}`, { method: 'DELETE' })
    ...
    .catch(() => mostrarMensaje('Error al eliminar el producto.', 'text-danger'));
}
...
// ═══════════════════════════════════════════════════════════════
// INICIALIZACIÓN — detecta en qué página estamos
// ═══════════════════════════════════════════════════════════════
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  // admin-productos.html — panel de gestión
