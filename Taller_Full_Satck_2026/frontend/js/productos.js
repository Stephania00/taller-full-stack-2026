...
function renderCatalogo(lista, filtro) {
    ...
    catalogo.innerHTML = datos.map(p => {
        const rutaImagen = p.imagen
            ? (p.imagen.startsWith('http') || p.imagen.startsWith('img/') ? p.imagen : `img/${p.imagen}`)
            : null;
        const placeholder = `<div class="ph ph-1-1"><span class="ph-label">[ ${p.categoria || 'Producto'} ]</span></div>`;
